document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleSearch = async () => {
        ...
        if (!currentSearchTerm) {
            // This logic is for resetting to browse mode after a search is cleared.
            // It replaces the pre-rendered content with dynamically loaded content.
            currentPage = 0; 
            allDataLoaded = false;
            loadingIndicator.style.display = 'none';
            loadBrowseItems();
    ...
    // Use a small timeout to ensure the pre-rendered content has fully settled in the DOM's layout calculation
    setTimeout(initialLoadCheck, 200);
});
